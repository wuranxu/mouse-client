...
func (s *RequestStat) stat(ctx context.Context) {
	...
	//go s.statScene(ctx)
	go s.record(ctx)
	go s.recordSuccess(ctx)
	go s.recordFailure(ctx)
...
func (s *RequestStat) record(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (s *RequestStat) recordSuccess(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (s *RequestStat) recordFailure(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
